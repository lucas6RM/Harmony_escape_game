...
- Exécute `npm run lint` pour vérifier les standards de code.
- Compare le code avec les standards du projet (`.opencode/CONTEXT.md`).
...
- Exécute `git add .` puis `git commit -m "issue N - tache M : [nom de la tâche]"` en lisant le nom de la tâche dans `.opencode/todo.md`.
- Réponds : `VALIDÉ`
...
- Exécute `git push origin HEAD` pour pousser la branche.
- Exécute `gh pr create --title "[titre]" --body "[description]" --label "ready-for-review"` pour créer la PR.
